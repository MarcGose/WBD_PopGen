...
col_palette <- c(wes_palette("Darjeeling1")[2],
...
ggsave("figures/FigS2.pdf", opt_k, height = 16.5, width = 21, units = "cm")
...
ngsAdmix_combined <- k1_plot + k2_plot + k3_plot + k4_plot + k5_plot + plot_layout(ncol = 1)

ggsave("figures/FigS6.pdf", ngsAdmix_combined, height = 16.5, width = 21, units = "cm")
...
ggsave("figures/Fig1d.pdf", plot3, height = 16.5, width = 21, units = "cm")

#NGSadmix only_WSINS

meta <- read.table("file_lists/popfile_BRITNS.txt", header = FALSE)
...
k3_plot_WSINS <- ggplot(K3, aes(factor(ID), value, fill = factor(name))) +
  geom_col(color = "gray", linewidth = 0.1, alpha = 0.7) +
  facet_grid(~fct_inorder(V3), switch = "x", scales = "free_x", space = "free") +
  theme_minimal() + 
  scale_fill_manual(values = col_palette) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_x_reordered(expand = expansion(add = 1.25)) +
...
ggsave("figures/Fig2c.pdf", k3_plot_WSINS, height = 5, width = 31, units = "cm")
